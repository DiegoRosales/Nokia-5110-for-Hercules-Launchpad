...
volatile bool textFormat = false;
...
static const byte ASCII[][5] = {
  ...
  ,{0x02, 0x04, 0x08, 0x10, 0x20} // 5c \ // Careful with accidentally commenting this one!
...
// Function that writes a character to the buffer
// on the specific x, y location (y is 1 - 6)
void bufferSetCharacter(byte character, byte x, byte y){
	...
	int i;
	for(i = 0; i < 5; i++){
		bufferSetByte(i+x, y, ASCII[character-0x20][i]);
	}
}
...
void bufferSetString(byte *string, byte size, byte x, byte y){
	...
	for(i = 0; i < size; i++)
	{
		...
		// If there was a text format character,
		// keep calm and carry on
		if(textFormat){
			textFormat = false;
		}
		else{
			bufferSetCharacter(*string, x + w, y);
			w++;
			x = x+5;
		}
		...
		// If the next character does not fit on the screen,
		// create a new line
		if(x + w > xLength-5)
		{
			x = 0;
